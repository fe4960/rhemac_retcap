...
cor=cor(new_data[,c(1:13)])
pdf("/storage/chenlab/Users/junwang/monkey/data/retcap/target_gene_with_var_new2_rm8_clinvar_simple_score_extend_all_cor_train.pdf",width=8)
diag(cor)=NA
Heatmap(cor)
dev.off()
...
#########
out_rhemac_af=iid_original1(data=new_data,pre="rhemac_AF" ,res="Y")
out_gnomad_af=iid_original1(data=new_data,pre="gnomad_AF" ,res="Y")
out_cadd=iid_original(data=new_data,pre="cadd" ,res="Y")
out_revel=iid_original(data=new_data,pre="revel" ,res="Y")
#out_sift=iid_original(data=new_data,pre="sift" ,res="Y")
out_polyphen2=iid_original(data=new_data,pre="polyphen2" ,res="Y")
out_mutationAssessor=iid_original(data=new_data,pre="mutationAssessor" ,res="Y")
out_MetaSVM_score=iid_original(data=new_data,pre="MetaSVM_score" ,res="Y")
out_MetaLR_score=iid_original(data=new_data,pre="MetaLR_score" ,res="Y")
out_phyloP100way_vertebrate=iid_original(data=new_data,pre="phyloP100way_vertebrate" ,res="Y")
#out_phyloP20way_mammalian=iid_original(data=new_data,pre="phyloP20way_mammalian" ,res="Y")
out_phastCons100way_vertebrate=iid_original(data=new_data,pre="phastCons100way_vertebrate" ,res="Y")
#out_phastCons20way_mammalian=iid_original(data=new_data,pre="phastCons20way_mammalian" ,res="Y")
#out_LRT=iid_original(data=new_data,pre="LRT" ,res="Y")
#out_MutationTaster=iid_original(data=new_data,pre="MutationTaster" ,res="Y")
#out_FATHMM=iid_original(data=new_data,pre="FATHMM" ,res="Y")
#out_PROVEAN=iid_original(data=new_data,pre="PROVEAN" ,res="Y")
out_VEST3=iid_original(data=new_data,pre="VEST3" ,res="Y")
#out_MutPred=iid_original(data=new_data,pre="MutPred" ,res="Y")
out_DANN=iid_original(data=new_data,pre="DANN" ,res="Y")
out_GERP=iid_original(data=new_data,pre="GERP" ,res="Y")

########
pdf("AUC_original_13score_HGMD_clinvar_missense_target_new_rm8_more.pdf")
par(mar = c(4, 6, 2, 2))
...
legend("bottomright",c(n2,n3,n4,n5,n7,n8,n9,n10,n11,n13,n19,n21,n22),lty=1,col=c("black","red","blue","olivedrab","mediumturquoise","grey","green","pink","yellow","lightgreen","lightblue","lightpink","thistle"),inset=0.05,lwd=2)
...
iid_example_model=function(data,V=5,pre,res,model){
 .cvFolds=function(Y,V){ #create CV folds (stratify by outcome)
 Y1=split(sample(which(Y=="p")),rep(1:V, length=length(which(Y=="p"))))
 Y0=split(sample(which(Y=="b")),rep(1:V, length=length(which(Y=="b"))))
 ...
 trainSet = data[-folds[[v]],]
 testSet = data[folds[[v]],]
 
  
 model_nb = train(trainSet[,1:pre],trainSet[,res],method=model, trControl=fitControl,tuneLength=7, metric="ROC" , preProcess = c('center','scale'))
 testSet$nb_stacked=predict(model_nb,testSet[,1:pre],type='prob')[,2]

  return(testSet$nb_stacked)
 ...
 #predictions=as.vector(sapply(seq(V),.doFit,folds=folds,data=data))
 predictions[unlist(folds)]=predictions
 out=ci.cvAUC(predictions=predictions,labels=data$Y,folds=folds,confidence=0.95)
 out1=cvAUC(predictions=predictions,labels=data$Y,folds=folds) 
 newlist=list("ciAUC"=out,"plotAUC"=out1)#logistical regression, bagging, random forest
 return(newlist) }
 
 ####################
 #logist regression
 ###########
 iid_example_lg=function(data,V=5,pre,res,model="glm"){
 .cvFolds=function(Y,V){ #create CV folds (stratify by outcome)
 Y1=split(sample(which(Y=="p")),rep(1:V, length=length(which(Y=="p"))))
 Y0=split(sample(which(Y=="b")),rep(1:V, length=length(which(Y=="b"))))
 ...
 model_nb = train(trainSet[,1:pre],trainSet[,res],method=model, family="binomial", trControl=fitControl,tuneLength=7, metric="ROC" , preProcess = c('center','scale'))
 testSet$nb_stacked=predict(model_nb,testSet[,1:pre],type='prob')[,2]
 ...
 predictions[unlist(folds)]=predictions
 out=ci.cvAUC(predictions=predictions,labels=data$Y,folds=folds,confidence=0.95)
 out1=cvAUC(predictions=predictions,labels=data$Y,folds=folds)  #logistical regression, bagging, random forest
 newlist=list("ciAUC"=out,"plotAUC"=out1)#logistical regression, bagging, random forest
 return(newlist) }
...
#neutral network
#############
iid_example_nn=function(data,V=5,pre,res,model="nnet"){
 .cvFolds=function(Y,V){ #create CV folds (stratify by outcome)
 Y1=split(sample(which(Y=="p")),rep(1:V, length=length(which(Y=="p"))))
 Y0=split(sample(which(Y=="b")),rep(1:V, length=length(which(Y=="b"))))
...
summaryFunction=twoClassSummary
 ...
 trainSet = data[-folds[[v]],]
 testSet = data[folds[[v]],]
 my.grid =expand.grid(.decay=c(0.5,0.1),.size=c(10,20))
  
 model_nb = train(trainSet[,1:pre],trainSet[,res],method=model, trControl=fitControl,tuneGrid=my.grid, trace=F, maxit=5000 ,metric="ROC", tuneLength=7 , preProcess = c('center','scale'))
 testSet$nb_stacked=predict(model_nb,testSet[,1:pre],type='prob')[,2]
 ...
 #predictions=as.vector(sapply(seq(V),.doFit,folds=folds,data=data))
 predictions[unlist(folds)]=predictions
 out=ci.cvAUC(predictions=predictions,labels=data$Y,folds=folds,confidence=0.95) #logistical regression, bagging, random forest
 out1=cvAUC(predictions=predictions,labels=data$Y,folds=folds)  #logistical regression, bagging, random forest
 newlist=list("ciAUC"=out,"plotAUC"=out1)#logistical regression, bagging, random forest
 return(newlist) }
...
#ensembl
#########
iid_example_es=function(data,V=5,pre,res){
 .cvFolds=function(Y,V){ #create CV folds (stratify by outcome)
 Y0=split(sample(which(Y=="b")),rep(1:V, length=length(which(Y=="b"))))
 Y1=split(sample(which(Y=="p")),rep(1:V, length=length(which(Y=="p"))))
 ...
 model_rf = train(trainSet[,1:pre],trainSet[,res],method='rf',trControl=fitControl,tuneLength=7, metric="ROC" , preProcess = c('center','scale'))
 trainSet$OOF_pred_rf = predict(model_rf,trainSet[,1:pre],type='prob')[,2]
 testSet$OOF_pred_rf = predict(model_rf,testSet[,1:pre],type='prob')[,2]

 model_knn = train(trainSet[,1:pre],trainSet[,res],method='knn',trControl=fitControl,tuneLength=7, metric="ROC" , preProcess = c('center','scale'))
 trainSet$OOF_pred_knn = predict(model_knn,trainSet[,1:pre],type='prob')[,2]

 testSet$OOF_pred_knn = predict(model_knn,testSet[,1:pre],type='prob')[,2]
 model_lr = train(trainSet[,1:pre],trainSet[,res],method='glm',trControl=fitControl,tuneLength=7, metric="ROC" , preProcess = c('center','scale'))
 trainSet$OOF_pred_lr = predict(model_lr,trainSet[,1:pre],type='prob')[,2]

 testSet$OOF_pred_lr = predict(model_lr,testSet[,1:pre],type='prob')[,2]

 model_gbm = train(trainSet[,predictors_top],trainSet[,res],method='nb', trControl=fitControl,tuneLength=7, metric="ROC" , preProcess = c('center','scale'))
 testSet$gbm_stacked=predict(model_gbm,testSet[,predictors_top],type='prob')[,2]
 ...
 #predictions=as.vector(sapply(seq(V),.doFit,folds=folds,data=data))
 predictions[unlist(folds)]=predictions
 out=ci.cvAUC(predictions=predictions,labels=data$Y,folds=folds) #logistical regression, bagging, random forest
 out1=cvAUC(predictions=predictions,labels=data$Y,folds=folds)  #logistical regression, bagging, random forest
 newlist=list("ciAUC"=out,"plotAUC"=out1)#logistical regression, bagging, random forest
 return(newlist) }
...
##################
######
iid_example_lm=function(data,V=5,pre,res,model){
 .cvFolds=function(Y,V){ #create CV folds (stratify by outcome)
 ...
 model_nb = train(trainSet[,1:pre],trainSet[,res],method=model, trControl=fitControl,tuneLength=7, metric="ROC" , preProcess = c('center','scale'))
 #testSet$nb_stacked=predict(model_nb,testSet[,1:pre],type='prob')[,2]
testSet$nb_stacked=predict(model_nb,testSet[,1:pre],type='raw')
  return(testSet$nb_stacked)
...
#new_data_c[,1:13]=new_data_st
out_rf=iid_example_model(data=new_data_c,V=5,pre=13,res=14,model="rf")
out_nb=iid_example_model(data=new_data_c,V=5,pre=13,res=14,model="nb")
out_lr=iid_example_lg(data=new_data_c,V=5,pre=13,res=14,model="glm")
out_tbg=iid_example_model(data=new_data_c,V=5,pre=13,res=14,model="treebag")
out_svm=iid_example_model(data=new_data_c,V=5,pre=13,res=14,model="svmLinear")
 out_nn=iid_example_nn(data=new_data_c,V=5,pre=13,res=14,model="nnet")
 out_es=iid_example_es(data=new_data_c,V=5,pre=13,res=14)
 
 #
 #out_lm=iid_example_lm(data=new_data,V=5,pre=14,res=15,model="lm")
 #
 ######
 #pdf("~/Documents/rhesus_paper/database/AUC_algorithm_performance_4feature_seed0.pdf")
 #pdf("~/Documents/rhesus_paper/retcap/AUC_algorithm_performance_8feature_seed0_HGMD_clinvar_missense_target.pdf")
 pdf("AUC_algorithm_performance_13feature_seed0_HGMD_clinvar_missense_target_new2_CV5_rm8.pdf")
 
 #pdf("~/Documents/rhesus_paper/retcap/AUC_original_16score_HGMD_clinvar_missense_target_new_rm8.pdf",width=8)
par(mar = c(4, 6, 2, 2))
# pdf("~/Documents/Clingene_Clinvar/AUC_algorithm_performance_9score_seed123.pdf")
# plot(out_rf$plotAUC$perf,col="black",main=paste0("5-fold CV AUC (patho", dim(new_data[new_data$Y==1,])[1], " vs. benign ", dim(new_data[new_data$Y==0,])[1],")"),lwd=2)
#plot(x=0,y=0,col="white",main=paste0("5-fold CV AUC (patho", dim(new_data[new_data$Y==1,])[1], " vs. benign ", dim(new_data[new_data$Y==0,])[1],")"),lwd=2,xlim=c(0,1),ylim=c(0,1))
plot(x=0,y=0,col="white",main="",lwd=2,xlim=c(0,1),ylim=c(0,1),ylab="TP rate", xlab="FP rate",cex.lab=2.5,cex.axis=2.5)
plot(out_rf$plotAUC$perf,col="black",avg="vertical",add=TRUE,lwd=2)

plot(out_nb$plotAUC$perf,col="red",avg="vertical",add=TRUE,lwd=2)
plot(out_lr$plotAUC$perf,col="blue",avg="vertical",add=TRUE,lwd=2)
plot(out_tbg$plotAUC$perf,col="olivedrab",avg="vertical",add=TRUE,lwd=2)
plot(out_svm$plotAUC$perf,col="orange",avg="vertical",add=TRUE,lwd=2)
plot(out_nn$plotAUC$perf,col="violet",avg="vertical",add=TRUE,lwd=2)
plot(out_es$plotAUC$perf,col="mediumturquoise",avg="vertical",add=TRUE,lwd=2)
#plot(out_lm$plotAUC$perf,col="grey",avg="vertical",add=TRUE,lwd=2)

n1=paste("Random forest (AUC= ",round(out_rf$plotAUC$cvAUC,3),")",sep="")
n2=paste("Naive bayes (AUC= ",round(out_nb$plotAUC$cvAUC,3),")",sep="")
n3=paste("Logistical regression (AUC= ",round(out_lr$plotAUC$cvAUC,3),")",sep="")
n4=paste("Tree Bagging (AUC= ",round(out_tbg$plotAUC$cvAUC,3),")",sep="")
n5=paste("SVM (AUC= ",round(out_svm$plotAUC$cvAUC,3),")",sep="")
n6=paste("Neural network (AUC= ",round(out_nn$plotAUC$cvAUC,3),")",sep="")

n7=paste("Ensemble (AUC= ",round(out_es$plotAUC$cvAUC,3),")",sep="")
#n8=paste("linear regression (AUC= ",round(out_lm$plotAUC$cvAUC,3),")",sep="")


legend("bottomright",c(n1,n2,n3,n4,n5,n6,n7),lty=1,col=c("black","red","blue","olivedrab","orange","violet","mediumturquoise","grey"),inset=0.05,lwd=2)
#legend("bottomright",c(n1,n2,n3,n4,n5,n7),lty=1,col=c("black","red","blue","olivedrab","orange","violet","mediumturquoise","grey"),inset=0.05,lwd=2)
...
# model_rf = train(new_data_c[,1:13],new_data_c[,14],method="rf", trControl=fitControl,tuneLength=7, metric="ROC") #,preProcess = c('center','scale'))
...
#data0=read.table("/storage/chenlab/Users/junwang/monkey/data/retcap/target_gene_with_var_new2_rm8_clinvar_simple_score_extend_all_flt",header=T,fill = TRUE)
data0=data0[!(data0$rhemac_coor%in%new_data$rhemac_coor),]
...
legend("bottomright",c(n2,n3,n4,n5,n7,n8,n9,n10,n19,n21,n22, n23),lty=1,col=c("black","red","blue","olivedrab","mediumturquoise","grey","green","pink","lightblue","lightpink","thistle","purple"),inset=0.05,lwd=2)
...
write.table(data1,file="/storage/chenlab/Users/junwang/monkey/data/retcap/target_gene_with_var_new2_rm8_swiss_simple_score_extend_all_rf_rank",sep="\t",quote=F,rownames=F)
